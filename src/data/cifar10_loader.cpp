...
// Get specific test batch by index (for deterministic access)
float *CIFAR10Loader::get_test_batch_at(int start_idx, int batch_size)
{
    if (!test_loaded_)
    {
        throw std::runtime_error("Test data not loaded. Call load_test_data() first.");
    }

    if (start_idx + batch_size > CIFAR_TEST_IMAGES)
    {
        batch_size = CIFAR_TEST_IMAGES - start_idx;
    }
    if (batch_size <= 0)
        return nullptr;
    ...
    // Copy images directly (no shuffling for test data)
    for (int i = 0; i < batch_size; i++)
    {
        std::memcpy(batch_buffer_ + i * CIFAR_PIXELS,
                    test_images_ + (start_idx + i) * CIFAR_PIXELS,
                    CIFAR_PIXELS * sizeof(float));
    }
...
// Get specific test batch labels by index (for deterministic access)
int *CIFAR10Loader::get_test_labels_at(int start_idx, int batch_size)
{
    if (!test_loaded_)
    {
        throw std::runtime_error("Test data not loaded. Call load_test_data() first.");
    ...
    for (int i = 0; i < batch_size; i++)
    {
        label_buffer_[i] = test_labels_[start_idx + i];
    }

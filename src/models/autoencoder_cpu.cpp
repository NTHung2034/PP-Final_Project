...
void AutoencoderCPU::free_activations() {
    delete[] act_conv1_; act_conv1_ = nullptr;
    delete[] act_relu1_; act_relu1_ = nullptr;
    delete[] act_pool1_; act_pool1_ = nullptr;
    delete[] act_conv2_; act_conv2_ = nullptr;
    delete[] act_relu2_; act_relu2_ = nullptr;
    delete[] act_pool2_; act_pool2_ = nullptr;
    
    delete[] act_conv3_; act_conv3_ = nullptr;
    delete[] act_relu3_; act_relu3_ = nullptr;
    delete[] act_up1_;   act_up1_ = nullptr;
    delete[] act_conv4_; act_conv4_ = nullptr;
    delete[] act_relu4_; act_relu4_ = nullptr;
    delete[] act_up2_;   act_up2_ = nullptr;
    delete[] act_conv5_; act_conv5_ = nullptr;
    
    delete[] grad_buffer_; grad_buffer_ = nullptr;
...
float* AutoencoderCPU::forward(const float* input, int batch) {
    ...
    float* x = conv1_->forward(input, batch, 32, 32);
    std::memcpy(act_conv1_, x, batch * 256 * 32 * 32 * sizeof(float));
    
    // ReLU1
    x = relu1_->forward(act_conv1_, batch * 256 * 32 * 32);
    std::memcpy(act_relu1_, x, batch * 256 * 32 * 32 * sizeof(float));
    
    // Pool1: [batch, 256, 32, 32] → [batch, 256, 16, 16]
    x = pool1_->forward(act_relu1_, batch, 256, 32, 32);
    std::memcpy(act_pool1_, x, batch * 256 * 16 * 16 * sizeof(float));
    
    // Conv2: [batch, 256, 16, 16] → [batch, 128, 16, 16]
    x = conv2_->forward(act_pool1_, batch, 16, 16);
    std::memcpy(act_conv2_, x, batch * 128 * 16 * 16 * sizeof(float));
    
    // ReLU2
    x = relu2_->forward(act_conv2_, batch * 128 * 16 * 16);
    std::memcpy(act_relu2_, x, batch * 128 * 16 * 16 * sizeof(float));
    
    // Pool2: [batch, 128, 16, 16] → [batch, 128, 8, 8] = LATENT
    x = pool2_->forward(act_relu2_, batch, 128, 16, 16);
    std::memcpy(act_pool2_, x, batch * 128 * 8 * 8 * sizeof(float));
    
    // ========================
    // DECODER
    // ========================
    // Conv3: [batch, 128, 8, 8] → [batch, 128, 8, 8]
    x = conv3_->forward(act_pool2_, batch, 8, 8);
    std::memcpy(act_conv3_, x, batch * 128 * 8 * 8 * sizeof(float));
    
    // ReLU3
    x = relu3_->forward(act_conv3_, batch * 128 * 8 * 8);
    std::memcpy(act_relu3_, x, batch * 128 * 8 * 8 * sizeof(float));
    
    // Up1: [batch, 128, 8, 8] → [batch, 128, 16, 16]
    x = up1_->forward(act_relu3_, batch, 128, 8, 8);
    std::memcpy(act_up1_, x, batch * 128 * 16 * 16 * sizeof(float));
    
    // Conv4: [batch, 128, 16, 16] → [batch, 256, 16, 16]
    x = conv4_->forward(act_up1_, batch, 16, 16);
    std::memcpy(act_conv4_, x, batch * 256 * 16 * 16 * sizeof(float));
    
    // ReLU4
    x = relu4_->forward(act_conv4_, batch * 256 * 16 * 16);
    std::memcpy(act_relu4_, x, batch * 256 * 16 * 16 * sizeof(float));
    
    // Up2: [batch, 256, 16, 16] → [batch, 256, 32, 32]
    x = up2_->forward(act_relu4_, batch, 256, 16, 16);
    std::memcpy(act_up2_, x, batch * 256 * 32 * 32 * sizeof(float));
    
    // Conv5: [batch, 256, 32, 32] → [batch, 3, 32, 32] = OUTPUT
    x = conv5_->forward(act_up2_, batch, 32, 32);
    std::memcpy(act_conv5_, x, batch * 3 * 32 * 32 * sizeof(float));
...
float* AutoencoderCPU::extract_features(const float* input, int batch) {
    ...
    // Run encoder only
    float* x = conv1_->forward(input, batch, 32, 32);
    x = relu1_->forward(x, batch * 256 * 32 * 32);
    x = pool1_->forward(x, batch, 256, 32, 32);
    
    x = conv2_->forward(x, batch, 16, 16);
    x = relu2_->forward(x, batch * 128 * 16 * 16);
    x = pool2_->forward(x, batch, 128, 16, 16);
    
    // Copy to activation buffer and return
    std::memcpy(act_pool2_, x, batch * 128 * 8 * 8 * sizeof(float));
    return act_pool2_;
}

...
public:
    ...
    // Returns encoded features (8x8x128 = 8192 dimensions)
    void forward_inference(const GPUTensor& input, GPUTensor& output) {
        ...
        // ENCODER
        conv2d_forward_gpu(input, *conv1, act1, 3, 3, 1, 1, true, stream_compute);      // Conv1 + ReLU
        
        // Need temporary storage for pooling in inference
        int* temp_pool1_idx;
        CUDA_CHECK(cudaMalloc(&temp_pool1_idx, batch * 256 * 16 * 16 * sizeof(int)));
        maxpool2d_forward_gpu(act1, act2, temp_pool1_idx, stream_compute);              // Pool1
        
        conv2d_forward_gpu(act2, *conv2, act3, 3, 3, 1, 1, true, stream_compute);       // Conv2 + ReLU
        
        int* temp_pool2_idx;
        CUDA_CHECK(cudaMalloc(&temp_pool2_idx, batch * 128 * 8 * 8 * sizeof(int)));
        maxpool2d_forward_gpu(act3, act4, temp_pool2_idx, stream_compute);              // Pool2
        
        // DECODER
        conv2d_forward_gpu(act4, *conv3, act5, 3, 3, 1, 1, true, stream_compute);       // Conv3 + ReLU
        upsample2d_forward_gpu(act5, act6, 2, stream_compute);                          // Upsample1
        conv2d_forward_gpu(act6, *conv4, act7, 3, 3, 1, 1, true, stream_compute);       // Conv4 + ReLU
        upsample2d_forward_gpu(act7, act8, 2, stream_compute);                          // Upsample2
        conv2d_forward_gpu(act8, *conv5, output, 3, 3, 1, 1, false, stream_compute);    // Conv5 (no ReLU)
    ...
    //  Extract Features - Returns latent representation (8x8x128)
    //  This is used for SVM training
    ...
    // Keeps all activations for backward pass
    float forward_backward_update(
        const GPUTensor& input,
        const GPUTensor& target,
        float learning_rate,
        std::vector<GPUTensor*>& activations)  // Preallocated activation buffers
    {
        int batch = input.batch;
        ...
        // BACKWARD PASS
        // Gradient buffers (reuse for memory efficiency)

...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        for (int b = 0; b < train_batches; ++b)
        {
            ...
            float *batch_data = loader.get_batch(current_batch_size);

            // Convert to Tensor
            Tensor batch_tensor({current_batch_size, 3, 32, 32}, false);
            std::memcpy(batch_tensor.raw_data(), batch_data, current_batch_size * 3 * 32 * 32 * sizeof(float));
        ...
        for (int b = 0; b < test_batches; ++b)
        {
            ...
            // Get test batch
            float *batch_data = loader.get_test_batch_at(b * BATCH_SIZE, current_batch_size);

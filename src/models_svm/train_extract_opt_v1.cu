...
int main(int argc, char** argv) {
    try {
        ...
        std::string weights_dir = MODEL_SAVE_DIR_GPU_OPT_V1;
        if (argc >= 2) data_dir = argv[1];

        std::cout << "\n";
        std::cout << "╔════════════════════════════════════════════════════════════════╗\n";
        std::cout << "║   CIFAR-10 FEATURE EXTRACTION - GPU OPT V1 (PHASE 4)          ║\n";
        std::cout << "╚════════════════════════════════════════════════════════════════╝\n";
        std::cout << "\n";
        ...
        AutoencoderGPUOptV1 autoencoder(feature_batch_size);
        
        // Load weights helper (adapted for opt v1)
        ...
        for (int batch_idx = 0; batch_idx < num_train_batches; batch_idx++) {
            ...
            conv2d_forward_opt_v1(autoencoder.input_buffer, *autoencoder.conv1, 
                                 autoencoder.pool.act1, true);  // Conv1+ReLU
            maxpool2d_forward_opt_v1(autoencoder.pool.act1, autoencoder.pool.act2, 
                                    autoencoder.pool.pool1_idx); // Pool1
            conv2d_forward_opt_v1(autoencoder.pool.act2, *autoencoder.conv2, 
                                 autoencoder.pool.act3, true);  // Conv2+ReLU
            maxpool2d_forward_opt_v1(autoencoder.pool.act3, autoencoder.pool.act4, 
